...
1. 物理页引用计数数组，让`kalloc()`和`kfree()`支持引用计数，避免把仍被共享的页面提前回收：

1.1 在 `kernel/kalloc.c` 顶部新增  
```c
struct {
  struct spinlock lock;
  int   count[PHYSTOP / PGSIZE];   // 每 4K 页一个计数
} ref;
...
kinit()
{
  ...
  memset(ref.count, 0, sizeof(ref.count));
  ... // 原有 kinit 代码
}
```

1.2 修改 `kalloc()`  
- 分配成功后把该页的 `ref.count[PA/PGSIZE]` 设为 1。  
...
1.3 修改 `kfree()`  
- 只有当引用计数为 0 时才真正放回 free list；否则仅递减计数。  
...
2. 修改 uvmcopy() —— 让父子共享只读映射  
文件：`kernel/vm.c` 中的 `uvmcopy()`。

- 不分配新物理页，而是把父进程的页表项PTE直接复制到子进程页表。  
- 清除 `PTE_W`，增加自定义标记 `PTE_COW`。  
- 因为父进程也是共享者之一，其也要清除 `PTE_W` 并加 `PTE_COW`。  
- 对引用计数+1。
...
for(i = 0; i < sz; i += PGSIZE){
  ...
  if(flags & PTE_W){          // 原来是可写的页
    *pte &= ~PTE_W;
    *pte |= PTE_COW;          // 设置 COW 标志
    kaddref((void*)pa);
  } else {                    // 原来是只读的页
    *pte |= PTE_COW;          // 可选：标记为共享只读
    kaddref((void*)pa);
  }
...
在 `kernel/riscv.h` 末尾增加：
```c
#define PTE_COW (1L << 8)
```

3. 处理缺页异常 —— usertrap()  
文件：`kernel/trap.c` 中的 `usertrap()`。

3.1 判断是 store page fault  
```c
if(r_scause() == 15 || r_scause() == 13){
  ...
  // 进一步判断 va 是否在用户空间有效范围
```

3.2 查询 PTE，验证 COW 标志  
```c
...
if(mappages(p->pagetable, PGROUNDDOWN(va), PGSIZE, (uint64)new, flags) != 0){
  kfree(new);
  p->killed = 1;
  goto end;
}
...
4. 处理 copyout() 的 COW 场景  
文件：`kernel/vm.c` 中的 `copyout()`，内核把数据复制到用户空间时也可能遇到 COW 页。

- 先查 PTE，如果 `PTE_COW` 置位，就按 3.3 的逻辑分配新页并替换映射，再写入。  
- 代码结构与 usertrap 基本相同。
```c
// 处理 COW 页
